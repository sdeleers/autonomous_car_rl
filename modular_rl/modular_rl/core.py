...
def run_policy_gradient_algorithm(env, agent, usercfg=None, callback=None):
    ...
    for _ in xrange(cfg["n_iter"]):
        # Rollouts ========
        start_time_get_paths = time.time()
        paths = get_paths(env, agent, cfg, seed_iter)
        print("--- get_paths:  %s seconds ---" % (time.time() - start_time_get_paths))
        # h5f = h5py.File('/Users/simon/MachineLearning/Results_car/results_03_3sens/172.31.4.225.a', 'r')
        # a = cPickle.loads(h5f['agent_snapshots'].values()[-1].value)
        # compute_advantage(agent.baseline, paths, gamma=cfg["gamma"], lam=cfg["lam"])
        compute_advantage(agent.baseline, paths, gamma=cfg["gamma"], lam=cfg["lam"])
        # VF Update ========
        start_time_baseline_fit = time.time()
        vf_stats = agent.baseline.fit(paths)
        print("--- baseline fit:  %s seconds ---" % (time.time() - start_time_baseline_fit))
        # Pol Update ========
        start_time_agent_update = time.time()
        pol_stats = agent.updater(paths)
        print("--- agent update:  %s seconds ---" % (time.time() - start_time_agent_update))
        # Stats ========
        stats = OrderedDict()
...
def rollout_two_agents(env, agent, timestep_limit):
    """
    ...
    for _ in xrange(timestep_limit):
        ob1 = agent.obfilt(ob[0:ob_len/2])
        ob2 = agent.obfilt(ob[ob_len/2:ob_len])
        ob = np.hstack((ob1, ob2))
        data["observation"].append(ob)
        action1, agentinfo1 = agent.act(ob1)
        action2, agentinfo2 = agent.act(ob2)
        agentinfo = {}
        agentinfo['prob'] = np.hstack((agentinfo1['prob'], agentinfo2['prob']))
        action = 3*action1+action2
...
class StochPolicy(object):
    ...
    def act(self, ob, stochastic=True):
        ...
        no_sensors = 0
        prob = self._act_prob(np.expand_dims(ob[0:ob.shape[0]-1-no_sensors], 0),
                              np.expand_dims(ob[ob.shape[0]-1-no_sensors:ob.shape[0]], 0))
        if stochastic:
